...
\subsection{傅里叶级数与滤波器}
\begin{itemize}
    \item \textbf{问题}：为什么在截止频率较高时，方波的合成波形会出现吉布斯现象？
    \item \textbf{解答}：吉布斯现象是由于傅里叶级数在不连续点（方波的跳变沿）附近的截断效应引起的。当有限项傅里叶级数去逼近不连续函数时，在不连续点附近会产生高频振荡，且振荡幅度不随项数增加而减小（约为跳变幅度的 9\%）。在本实验中，低通滤波器相当于截断了高频谐波，因此在波形跳变处观察到了明显的过冲和振荡。
\end{itemize}

\subsection{一阶系统时域响应}
\begin{itemize}
    \item \textbf{问题}：RC 电路的时间常数 \(\tau\) 对系统阶跃响应有何影响？
    \item \textbf{解答}：时间常数 \(\tau = RC\) 反映了电路充放电的快慢。实验中，\(\tau\) 越大，电容充电越慢，阶跃响应的上升时间越长，系统反应越迟钝；反之，\(\tau\) 越小，响应越快。
\end{itemize}

\subsection{采样与混叠}
\begin{itemize}
    \item \textbf{问题}：发生混叠后，能否通过滤波恢复原始信号？为什么？
    \item \textbf{解答}：一旦发生混叠，高频信号就会“伪装”成低频信号（折叠到第一奈奎斯特区），此时混叠分量与真实的低频分量在频域上重叠，无法通过普通的线性滤波器区分和滤除。因此，混叠是不可逆的，必须在采样前使用抗混叠滤波器（Anti-aliasing Filter）来限制输入信号的带宽。
\end{itemize}
